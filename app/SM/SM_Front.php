...
trait SM_Front
{
            ...
            if (count($tagsArray) > 0) {
                foreach ($tagsArray as $tg) {
                    ...
                    if ($tag) { 
                        ...
                        $tag = new Tag();
                        $tag->title = $tg;
                        $tag->slug = SM::create_uri("tags", $tg);
     ...
     */
    public static function sm_image_upload($f_name, $v_rules, $isPrivate = false, $flag)
    {
        ...
        if ($validator->passes()) {
            ...
            if ($isPrivate) {
                ...
                if (
                    ...
                    self::sm_image_resize($all_width, $all_height, $fullPath, $filename, $isPrivate, $flag);
                }
            } else {
                ...
                if (
                    ...
                    $all_width = config('constant.smImgWidth');
                    $all_height = config('constant.smImgHeight');
                    self::sm_image_resize($all_width, $all_height, storage_path("app/public/" . $path), $filename, $isPrivate, $flag);
                }
            }
     ...
     */
    public static function sm_image_resize($all_width, $all_height, $path, $filename, $isPrivate = false, $flag)
    {
        ...
        if (
            ...
            $watermarkImagePath = $path . '/watermark-for-durbiin-book.png';
                    ...
                    if ($flag == 1) {
                        ...
                        switch ($fileType) {
                            case 'jpg':
                                ...
                                $im = imagecreatefromjpeg($fileOptimize);
                            ...
                            case 'jpeg':
                                ...
                                break;
                            case 'png':
                                $im = imagecreatefrompng($fileOptimize);
                                break;
                            default:
                                $im = imagecreatefromjpeg($fileOptimize);
                        }
                        ...
                        $marge_right = (int) (($width * 10) / 100);
                        $marge_bottom = (int) (($height * 10) / 100);
...
<?php
    ...
    public static function getSpecialProduct($noOfPost = 7)
    {
        $dis_pro = array();
        $products = Product::published()->get();
        // dd($products);
        foreach ($products as $key => $value) {
            if ($value->regular_price > 0 && $value->sale_price > 0) {
                $regular_price = $value->regular_price;
                $sale_price = $value->sale_price;
                $discount_percentage = (int)(($regular_price - $sale_price) * 100) / $regular_price;
                if ($discount_percentage > 26) {
                    array_push($dis_pro, $value->id);
                }
            }
        }

        return SM::getCache('product_sidebar_special', function () use ($noOfPost, $dis_pro) {
    ...
    public static function getBestSaleProduct($noOfProduct = 7)
    {
    ...
    public static function getBestSaleProductByCategory($slug = null)
    {
    ...
    public static function getBestSaleProductWeek($noOfProduct = 20)
    {

        $bestSale = SM::getCache('product_detail_best_sale', function () use ($noOfProduct) {

            $date1 =  Carbon\Carbon::now();
            $date2 = Carbon\Carbon::now()->subDays(7);
            ...
            return DB::table('products')
                ...
                ->groupBy('products.id')
                ->limit(20)
                ->whereBetween('order_details.created_at', [$date2, $date1])
                ->get();
        });
    ...
    public static function addToCartButton($product_id = null, $regular_price = null, $sale_price = null, $addClass = null)
    {
        ...
        if (!empty($item)) {
            ...
            if (!empty($product) && $product->product_qty > 0) {
