    ...
    function login(Request $request) {
        ...
        if ($validator->fails()) {
            if ($request->expectsJson()) {
                ...
                return redirect("login_signin_new")->with("errors", $validator->errors())->withInput();
            }
        }
        ...
        if ($this->attemptLogin($request)) {
            ...
            if (Session::has('is_pending_user') && Session::get('is_pending_user') == 1) {
                if ($request->expectsJson()) {
                    ...
                    return redirect('login_signin_new')->withInput()->with('s_message', 'Please wait for Account admin Approval!');
                }
            }else{
                return redirect('login_signin_new')->withInput()->with('w_message', 'Invalid username or password!!');
            }
        }
...
//		$request->session()->regenerate();
        ...
        return redirect('/login_signin_new')->with('s_message', 'Successfully logged Out');
    }
